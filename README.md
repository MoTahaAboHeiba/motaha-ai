...
FastAPI + Uvicorn
  ...
  └── RAG pipeline
        ...
        ├── Generation
        │     ├── Groq: openai/gpt-oss-20b, streaming
...
Run this after adding or changing knowledge-base Markdown or PDF files:
...
1. Reads every `.md` and text-based `.pdf` file under `knowledge_base/`.
2. Extracts PDF text with PyMuPDF.
3. Splits documents into 512-character chunks with 64-character overlap.
4. Embeds documents with Gemini `gemini-embedding-001`.
5. Creates or validates the `motaha-ai` Qdrant collection.
6. Upserts deterministic point IDs into Qdrant.

Place your CV and LinkedIn export PDFs anywhere under `knowledge_base/`, then
run the ingestion command again. Image-only or scanned PDFs need OCR first;
PyMuPDF extracts selectable text but does not perform OCR.
