...
Mohamed Taha built EduMate RAG to provide grounded answers from indexed
university course materials for the EduMate application.
...
A Retrieval-Augmented Generation (RAG) service for EduMate. It runs as a FastAPI microservice behind the main EduMate .NET backend for the Flutter app, with Qdrant Cloud in production and ChromaDB in development. It uses hybrid BM25 and semantic retrieval with a Qwen3-0.6B reranker and Groq Llama 3.3-70B generation.

The portfolio reports an evaluation set of 73 question-answer pairs, 79.45% HitRate@5, MRR of 0.75, 100% source accuracy, approximately 240 ms latency, and 5,637 vectors in the production Qdrant store. The deployed service is https://mo-taha-aboheiba-edumate-rag.hf.space. The .NET backend integration contract is `POST /api/integrations/query`, and the vector backend is selected with the `VECTOR_STORE_BACKEND` environment variable.
...
- **Hybrid Retrieval** - BM25 and semantic search retrieve relevant PDF chunks
- **Cross-encoder Reranking** - Retrieved candidates are reranked before generation
...
```
           ...
           [VECTOR_STORE_BACKEND=chroma]                [VECTOR_STORE_BACKEND=qdrant]
             ...
             - Development and evaluation                   - Production persistence
             - Local retrieval                              - HTTPS connection
                       │                                             │
                       └──────────────────────┬──────────────────────┘
                                              ▼
...
Production deployment facts: the production vector store contains 5,637
vectors, and the deployment is available at
https://mo-taha-aboheiba-edumate-rag.hf.space. The dual-backend toggle is
controlled by the `VECTOR_STORE_BACKEND` environment variable.
...
GROQ_MODEL=llama-3.3-70b-versatile          # Llama 3.3-70B generation
